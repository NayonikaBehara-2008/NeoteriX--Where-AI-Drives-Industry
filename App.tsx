...
const App: React.FC = () => {
  ...
  const getTranslation = () => {
    ...
    const selected = translations[user.language] || {};
    
    // Deep merge top-level categories to ensure sub-keys exist
    return {
      ...base,
      ...selected,
      nav: { ...base.nav, ...(selected.nav || {}) },
      home: { ...base.home, ...(selected.home || {}) },
      audit: { ...base.audit, ...(selected.audit || {}) },
      community: { ...base.community, ...(selected.community || {}) },
      messages: { ...base.messages, ...(selected.messages || {}) },
      profile: { ...base.profile, ...(selected.profile || {}) },
      dashboard: { ...base.dashboard, ...(selected.dashboard || {}) },
    };
  };
